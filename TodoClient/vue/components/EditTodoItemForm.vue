<template>
  <b-modal :id="`modal-${todoListItem.id}`" :title="todoListItem.name" @shown="refresh">
    <!-- Name -->
    <b-form @submit.prevent="sendItemEditedEvent">
      ...
      <b-button type="submit" variant="success" size="sm" v-if="editingName" class="mb-2">Save</b-button>
      <b-button
        variant="secondary"
        size="sm"
        v-if="editingName"
    ...
    <b-form @submit.prevent="sendItemEditedEvent">
      <b-form-group label="Notes">
        <b-form-textarea rows="3" maxlength="200" v-model="form.notes" @focus="editingNotes = true"></b-form-textarea>
      </b-form-group>
      <b-button type="submit" variant="success" size="sm" v-if="editingNotes" class="mb-2">Save</b-button>
      <b-button
        variant="secondary"
        size="sm"
        v-if="editingNotes"
    ...
    <b-form @submit.prevent v-if="plan.canAddDueDates">
      <b-form-group>
        <label for="due-date">
          Due Date:
          <span v-if="form.dueDate">{{ form.dueDate | formatDate }}</span>
        </label>
        <b-form-datepicker id="due-date" v-model="form.dueDate"></b-form-datepicker>
      </b-form-group>
    </b-form>
