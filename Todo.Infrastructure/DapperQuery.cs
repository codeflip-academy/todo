    ...
    public class DapperQuery
    {
        ...
        public async Task<EmailFilterDto> GetEmailFilterAsync(Guid accountId)
        {
            using (var connection = new SqlConnection(_connectionString))
            {
                ...
                var result = await connection.QueryAsync<EmailFilterDto>("SELECT EmailDueDate, EmailListCompleted, EmailItemCompleted, EmailInvitation FROM Accounts Where ID = @accountId", new { accountId = accountId });
                return result.FirstOrDefault();
            }
        }
